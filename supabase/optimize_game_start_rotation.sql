-- The quality pass retired duplicate records, so the hot game-start path can
-- rotate by indexed question_id without recalculating content hashes for every
-- row in the 60k-question archive.
...
begin
  ...
  for v_step in select * from public.round_template_steps where template_id=v_template.id order by position loop
    for v_question in
      ...
      where q.status='approved' and q.game_mode=v_template.game_mode
        ...
        and not exists(select 1 from public.game_rounds gr where gr.room_id=p_room_id and gr.question_id=q.id)
...
-- Cover the joins used repeatedly by game-state and cleanup operations.
create index if not exists game_rounds_question_idx on public.game_rounds(question_id);
create index if not exists game_rounds_template_idx on public.game_rounds(template_id);
create index if not exists player_answers_room_idx on public.player_answers(room_id);
create index if not exists player_answers_selected_option_idx on public.player_answers(selected_option_id);
create index if not exists round_option_orders_option_idx on public.round_option_orders(option_id);
